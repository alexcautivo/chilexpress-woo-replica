...
set +e
docker compose -f docker-compose.local.yml exec -T wpcli wp plugin install woocommerce --version=11.0.1 --force --activate
status=$?
set -e
...
docker compose -f docker-compose.local.yml logs --tail=40 wordpress
exit "$status"
